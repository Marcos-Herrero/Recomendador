...
import conexion_a_base; from conexion_a_base import *
...
def get_ovstock_group_quantity(data_stock, unique_values):
	...
	df_gstock = data_stock.groupby(['articulo', 'deposito', 'periodo']).agg({'cajas': 'sum'})
...
def get_useful_life(data_stock, unique_values):
	...
	vu_periodo = (1/data_stock.groupby(['deposito', 'periodo']).agg({'vida_util': 'mean'})).round(5)
...
def GetLotesXArtPerDep(aux, newStock, solDict):
	...
	for NordenVenta, cantidad in solDict.items():
		...
		if cantidad==0:
			...
			res = greedyAlg(newStock,(round(cantidad/2)))
...
def performance_antes_desp(data_stock,data_stock_N):
	...
	Vence10Dias = data_stock.query('vida_util<130')['cajas'].sum()
	Vence30Dias = data_stock.query('vida_util<150')['cajas'].sum()
	Vence90Dias = data_stock.query('vida_util<210')['cajas'].sum()
	Vence180Dias = data_stock.query('vida_util<300')['cajas'].sum()
	

	Vence10DiasN = data_stock_N.query('vida_util<130')['cajas'].sum()
	Vence30DiasN = data_stock_N.query('vida_util<150')['cajas'].sum()
	Vence90DiasN = data_stock_N.query('vida_util<210')['cajas'].sum()
	Vence180DiasN = data_stock_N.query('vida_util<300')['cajas'].sum()
...
def algoritmo_principal(data_ov, data_stock, cant_depositos, cumplimiento):
	print("ENTRA A ALGORITMO PRINCIAL")
